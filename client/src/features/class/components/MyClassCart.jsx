  ...
  return (
    <div className="space-y-6 pb-10">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Tổng số lớp', value: totalClasses, icon: <Activity size={18} />, bg: 'rgba(249,115,22,0.1)', color: '#ea580c' },
          { label: 'Học viên tham gia', value: activeStudents, icon: <Users size={18} />, bg: 'rgba(245,158,11,0.1)', color: '#d97706' },
          { label: 'Lớp sắp mở', value: upcomingClasses, icon: <Clock size={18} />, bg: 'rgba(59,130,246,0.1)', color: '#2563eb' },
          { label: 'Đang mở đăng ký', value: openClasses, icon: <ShieldAlert size={18} />, bg: 'rgba(16,185,129,0.1)', color: '#059669' }
        ].map((stat, idx) => (
          <div
            key={idx}
        ...
        >
          ...
          <div className="relative z-10 shrink-0 w-full md:w-auto">
            <Link to={nextClass.meetingLink || '#'}>
              ...
              >
                <PlayCircle size={18} className="group-hover:rotate-12 transition-transform" /> Vào Phòng Học ▶
              </button>
            </Link>
          </div>
        </section>
      ...
      >
        <h3 className="text-lg font-bold text-slate-800 px-2">Danh Sách Lớp Học</h3>
        ...
        <div className="flex flex-wrap gap-2 w-full md:w-auto">
          ...
          <select
            className="w-full sm:w-36 glass-input py-2 text-xs cursor-pointer"
            onChange={(e) => setFilterday(e.target.value)}
          >
            <option value="All Day">Tất cả các ngày</option>
              ...
              <div className="p-5 flex flex-col flex-1">
                ...
                <div className="flex justify-between items-start mb-3">
                  <div className="flex-1 pr-2">
                    ...
                    >
                      ● {item.status === 'open' ? 'Đang mở' : item.status === 'upcoming' ? 'Sắp mở' : 'Đã đóng'}
                    </span>
                  ...
                  <div className="relative">
                    <button
                      className="p-1.5 rounded-full hover:bg-white/70 transition-colors text-slate-400 hover:text-slate-700 cursor-pointer"
                    ...
                    {dropdownOpen === item._id && (
                      <div
                        className="absolute right-0 top-9 w-44 rounded-2xl py-2 z-50 shadow-xl"
                        style={{
                      ...
                      >
                        <button
                          className="w-full text-left px-4 py-2 text-xs font-semibold text-slate-700 hover:bg-orange-50 flex items-center gap-2 cursor-pointer"
                          onClick={() => navigate(`/update-class/${item?._id}`, { state: { data: item } })}
                        >
                          <Edit size={13} /> Sửa thông tin
                        </button>
                        <button
                          className="w-full text-left px-4 py-2 text-xs font-semibold text-slate-700 hover:bg-orange-50 flex items-center gap-2 cursor-pointer"
                          onClick={() => handchangesStatus(item)}
                        >
                          <Settings size={13} /> Đổi trạng thái
                        </button>
                        <div className="h-[1px] bg-slate-100 my-1" />
                        <button className="w-full text-left px-4 py-2 text-xs font-semibold text-rose-600 hover:bg-rose-50 flex items-center gap-2 cursor-pointer">
                          <Trash2 size={13} /> Xóa lớp học
                        </button>
                      </div>
                    )}
                  </div>
                ...
                <div className="mb-4">
                  <div className="flex justify-between items-end mb-1.5">
                    <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Sĩ số học viên</span>
                ...
                <div className="flex flex-col gap-1.5 p-3 rounded-xl mb-5" style={{ background: 'rgba(255,255,255,0.45)', border: '1px dashed rgba(249,115,22,0.15)' }}>
                  <div className="flex items-center gap-2 text-xs font-semibold text-slate-600">
                    <Calendar size={13} className="text-orange-500" /> Khai giảng: {item.startDate}
                  </div>
                  <div className="flex items-center gap-2 text-xs font-semibold text-slate-600">
                    <Clock size={13} className="text-orange-500" /> {getDayLabel(item.schedule?.day)} • {item.schedule?.startTime} - {item.schedule?.endTime}
                  </div>
                </div>
                ...
                <div className="mt-auto flex flex-col gap-2">
                  <button
                    className="w-full py-2.5 rounded-full font-bold text-white text-xs transition-all duration-300 hover:scale-[1.02] hover:shadow-md flex items-center justify-center gap-2 cursor-pointer"
                    style={{
                      background: 'linear-gradient(135deg, #f97316, #fb923c)',
                  ...
                  >
                     <Activity size={13} /> Vào phòng học
                  </button>

                  <div className="grid grid-cols-2 gap-2">
                    <button
                      className="py-2 rounded-full text-xs font-bold transition-all hover:scale-[1.02] hover:bg-white flex items-center justify-center gap-1.5 cursor-pointer"
                      style={{
                    ...
                    >
                      <Users size={13} /> Danh sách học viên
                    </button>
                    ...
                    >
                      <PlayCircle size={15} /> Link phòng học
                    </button>
                  </div>
                </div>
              </div>
      ...
      {classs?.length === 0 && (
        <div className="flex flex-col items-center justify-center text-center p-12 rounded-[2rem]" style={{ background: 'rgba(255,255,255,0.45)', border: '1px dashed rgba(249,115,22,0.25)' }}>
          ...
          <h4 className="text-xl font-bold mb-1" style={{ color: '#1e293b' }}>Chưa Có Lớp Học Nào</h4>
          <p className="text-xs" style={{ color: '#64748b' }}>Hãy bắt đầu tạo lớp học trực tuyến đầu tiên của bạn.</p>
        </div>
      )}
    </div>
  );
